module.exports = function (app, swig, gestorBD, logger) {
     ...
     * Funcion para mostrar una oferta en concreto segun un id
     */
    app.get('/oferta/editar/:id', function (req, res) {
        let criterio = {"_id": gestorBD.mongo.ObjectID(req.params.id)};
        gestorBD.obtenerOfertas(criterio, function (ofertas) {
            if (ofertas == null) {
                res.redirect("/tienda" +
                ...
                let respuesta = swig.renderFile('views/beditaroferta.html',
                    {
                        oferta: ofertas[0],
                        user: req.session.usuario,
                        dinero: req.session.dinero,
     ...
     * Funcion para mostrar una oferta en concreto segun un id
     */
    app.post('/oferta/editar', function (req, res) {
        ...
        let criterio = {"_id": gestorBD.mongo.ObjectID(req.body.ident)};
        gestorBD.obtenerOfertas(criterio, function (ofertas) {
            if (ofertas == null) {
                res.redirect("/tienda" +
                    "?mensaje=Ha ocurrido un error inesperado al obtener ofertas" +
                ...
                let ofertaCopia = ofertas[0];
                ofertaCopia.nombre = nombre;
                ofertaCopia.detalles = detalles;
                ofertaCopia.precio = precio;
                console.log(ofertaCopia);

                gestorBD.insertarOferta(ofertaCopia, function (id) {
                    if (id == null) {
                        res.redirect("/tienda" +
